...
export default function TextToSlug() {
  ...
  const generateSlug = () => {
    if (!text.trim()) return;
    const generatedSlug = text.toLowerCase().replace(/[^\w\s-]/g, '').replace(/[\s_-]+/g, '-').replace(/^-+|-+$/g, '');
  ...
  const clearAll = () => { setText(''); setSlug(''); };

  return (
    <div className="space-y-6">
      <div>
        <label className="text-sm font-medium text-slate-700">Enter Text</label>
        <input type="text" value={text} onChange={(e) => setText(e.target.value)} placeholder="My Awesome Blog Post Title" className="input-field mt-1" onKeyDown={(e) => e.key === 'Enter' && generateSlug()} />
        <p className="mt-1 text-xs text-slate-400">Spaces, special characters, and uppercase will be converted.</p>
      </div>

      <div className="flex flex-wrap gap-3">
        <button onClick={generateSlug} className="btn-primary"><RefreshCw className="mr-2 h-4 w-4" /> Generate Slug</button>
      ...
      <div className="rounded-xl bg-slate-50 p-4 border border-slate-200/50">
        <p className="text-sm text-slate-500"><span className="font-semibold">Example:</span> &quot;My Blog Post!&quot; → &quot;my-blog-post&quot;</p>
      </div>
    </div>
  );
}