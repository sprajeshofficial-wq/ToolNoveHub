  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 gap-4">
        <button onClick={simulateScan} disabled={isScanning} className="flex flex-col items-center gap-2 rounded-2xl bg-gradient-to-r from-teal-500 to-cyan-500 p-6 text-white shadow-lg shadow-teal-500/25 transition-all hover:scale-105 disabled:opacity-50">
          <Camera className="h-8 w-8" />
          <span className="font-medium">Scan with Camera</span>
          <span className="text-xs text-white/70">Simulated for demo</span>
        </button>
        <button onClick={() => fileInputRef.current?.click()} disabled={isScanning} className="flex flex-col items-center gap-2 rounded-2xl bg-slate-100 p-6 text-slate-700 transition-all hover:bg-slate-200 disabled:opacity-50">
        ...
        <div className="rounded-2xl bg-emerald-50 p-4 border border-emerald-200">
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <button onClick={copyToClipboard} className="rounded-lg bg-emerald-100 px-3 py-1.5 text-sm font-medium text-emerald-700 transition-all hover:bg-emerald-200">{copied ? 'Copied!' : 'Copy'}</button>
              <button onClick={clearScan} className="rounded-lg bg-red-100 px-3 py-1.5 text-sm font-medium text-red-600 transition-all hover:bg-red-200"><X className="h-4 w-4" /></button>
            </div>
          </div>
        </div>
      )}
